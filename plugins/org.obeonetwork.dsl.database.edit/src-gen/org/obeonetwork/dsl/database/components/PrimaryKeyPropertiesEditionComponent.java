...
public class PrimaryKeyPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final PrimaryKeyPropertiesEditionPart basePart = (PrimaryKeyPropertiesEditionPart)editingPart;
			// init values
			if (primaryKey.getName() != null && isAccessible(DatabaseViewsRepository.PrimaryKey.Properties.name))
				basePart.setName(EEFConverterUtil.convertToString(EcorePackage.eINSTANCE.getEString(), primaryKey.getName()));
			...
			if (primaryKey.getComments() != null && isAccessible(DatabaseViewsRepository.PrimaryKey.Properties.comments))
				basePart.setComments(EcoreUtil.convertToString(EcorePackage.eINSTANCE.getEString(), primaryKey.getComments()));
			// init filters
			...
			basePart.addFilterToColumns(new ViewerFilter() {
				...
				public boolean select(Viewer viewer, Object parentElement, Object element) {
					if (element instanceof EObject)
						return (!basePart.isContainedInColumnsTable((EObject)element));
					return element instanceof String && element.equals("");
				}
			
			});
			basePart.addFilterToColumns(new EObjectStrictFilter(DatabasePackage.eINSTANCE.getColumn()));
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {	
			PrimaryKeyPropertiesEditionPart basePart = (PrimaryKeyPropertiesEditionPart)editingPart;
			if (DatabasePackage.eINSTANCE.getNamedElement_Name().equals(msg.getFeature()) && basePart != null && isAccessible(DatabaseViewsRepository.PrimaryKey.Properties.name)) {
			...
			if (DatabasePackage.eINSTANCE.getDatabaseElement_Comments().equals(msg.getFeature()) && basePart != null && isAccessible(DatabaseViewsRepository.PrimaryKey.Properties.comments)){
				if (msg.getNewValue() != null) {
