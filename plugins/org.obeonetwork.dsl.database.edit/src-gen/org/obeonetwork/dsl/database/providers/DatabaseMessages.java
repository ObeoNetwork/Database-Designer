/**
 * Generated with Acceleo
 */
package org.obeonetwork.dsl.database.providers;

import org.eclipse.osgi.util.NLS;

/**
 * 
 * 
 */
public class DatabaseMessages extends NLS {
	
	private static final String BUNDLE_NAME = "org.obeonetwork.dsl.database.providers.databaseMessages"; //$NON-NLS-1$

	
	public static String DataBasePropertiesEditionPart_PropertiesGroupLabel;

	
	public static String ColumnPropertiesEditionPart_PropertiesGroupLabel;

	
	public static String IndexPropertiesEditionPart_PropertiesGroupLabel;

	
	public static String ViewPropertiesEditionPart_PropertiesGroupLabel;

	
	public static String TablePropertiesEditionPart_PropertiesGroupLabel;

	
	public static String ForeignKeysPropertiesEditionPart_PropertiesGroupLabel;

	
	public static String IndexesPropertiesEditionPart_PropertiesGroupLabel;

	
	public static String ConstraintsPropertiesEditionPart_PropertiesGroupLabel;

	
	public static String PrimaryKeyPropertiesEditionPart_PropertiesGroupLabel;

	
	public static String ForeignKeyPropertiesEditionPart_PropertiesGroupLabel;

	
	public static String ForeignKeyElementPropertiesEditionPart_PropertiesGroupLabel;

	
	public static String ForeignKeyElementPropertiesEditionPart_SourceTableGroupLabel;

	
	public static String ForeignKeyElementPropertiesEditionPart_TargetTableGroupLabel;

	
	public static String IndexElementPropertiesEditionPart_PropertiesGroupLabel;

	
	public static String ConstraintPropertiesEditionPart_PropertiesGroupLabel;

	
	public static String SchemaPropertiesEditionPart_PropertiesGroupLabel;

	
	public static String SequencePropertiesEditionPart_PropertiesGroupLabel;

	
	public static String DataBase_ReadOnly;

	
	public static String DataBase_Part_Title;

	
	public static String Column_ReadOnly;

	
	public static String Column_Part_Title;

	
	public static String Index_ReadOnly;

	
	public static String Index_Part_Title;

	
	public static String View_ReadOnly;

	
	public static String View_Part_Title;

	
	public static String Table_ReadOnly;

	
	public static String Table_Part_Title;

	
	public static String ForeignKeys_ReadOnly;

	
	public static String ForeignKeys_Part_Title;

	
	public static String Indexes_ReadOnly;

	
	public static String Indexes_Part_Title;

	
	public static String Constraints_ReadOnly;

	
	public static String Constraints_Part_Title;

	
	public static String PrimaryKey_ReadOnly;

	
	public static String PrimaryKey_Part_Title;

	
	public static String ForeignKey_ReadOnly;

	
	public static String ForeignKey_Part_Title;

	
	public static String ForeignKeyElement_ReadOnly;

	
	public static String ForeignKeyElement_Part_Title;

	
	public static String IndexElement_ReadOnly;

	
	public static String IndexElement_Part_Title;

	
	public static String Constraint_ReadOnly;

	
	public static String Constraint_Part_Title;

	
	public static String Schema_ReadOnly;

	
	public static String Schema_Part_Title;

	
	public static String Sequence_ReadOnly;

	
	public static String Sequence_Part_Title;

	
	public static String DataBasePropertiesEditionPart_NameLabel;

	
	public static String DataBasePropertiesEditionPart_UrlLabel;

	
	public static String DataBasePropertiesEditionPart_UsedLibrariesLabel;

	
	public static String DataBasePropertiesEditionPart_CommentsLabel;

	
	public static String ColumnPropertiesEditionPart_NameLabel;

	
	public static String ColumnPropertiesEditionPart_TypeLabel;

	
	public static String ColumnPropertiesEditionPart_LengthLabel;

	
	public static String ColumnPropertiesEditionPart_PrecisionLabel;

	
	public static String ColumnPropertiesEditionPart_LiteralsLabel;

	
	public static String ColumnPropertiesEditionPart_NullableLabel;

	
	public static String ColumnPropertiesEditionPart_PrimaryKeyLabel;

	
	public static String ColumnPropertiesEditionPart_UniqueLabel;

	
	public static String ColumnPropertiesEditionPart_AutoincrementLabel;

	
	public static String ColumnPropertiesEditionPart_SequenceLabel;

	
	public static String ColumnPropertiesEditionPart_DefaultValueLabel;

	
	public static String ColumnPropertiesEditionPart_CommentsLabel;

	
	public static String IndexPropertiesEditionPart_NameLabel;

	
	public static String IndexPropertiesEditionPart_QualifierLabel;

	
	public static String IndexPropertiesEditionPart_UniqueLabel;

	
	public static String IndexPropertiesEditionPart_CardinalityLabel;

	
	public static String IndexPropertiesEditionPart_IndexTypeLabel;

	
	public static String IndexPropertiesEditionPart_ElementsLabel;

	
	public static String IndexPropertiesEditionPart_CommentsLabel;

	
	public static String ViewPropertiesEditionPart_NameLabel;

	
	public static String ViewPropertiesEditionPart_QueryLabel;

	
	public static String ViewPropertiesEditionPart_CommentsLabel;

	
	public static String TablePropertiesEditionPart_NameLabel;

	
	public static String TablePropertiesEditionPart_ColumnsLabel;

	
	public static String TablePropertiesEditionPart_CommentsLabel;

	
	public static String ForeignKeysPropertiesEditionPart_ForeignKeysLabel;

	
	public static String IndexesPropertiesEditionPart_IndexesLabel;

	
	public static String ConstraintsPropertiesEditionPart_ConstraintsLabel;

	
	public static String PrimaryKeyPropertiesEditionPart_NameLabel;

	
	public static String PrimaryKeyPropertiesEditionPart_ColumnsLabel;

	
	public static String PrimaryKeyPropertiesEditionPart_CommentsLabel;

	
	public static String ForeignKeyPropertiesEditionPart_NameLabel;

	
	public static String ForeignKeyPropertiesEditionPart_TargetLabel;

	
	public static String ForeignKeyPropertiesEditionPart_ElementsLabel;

	
	public static String ForeignKeyPropertiesEditionPart_CommentsLabel;

	
	public static String ForeignKeyElementPropertiesEditionPart_SourceTableLabel;

	
	public static String ForeignKeyElementPropertiesEditionPart_FKColumnLabel;

	
	public static String ForeignKeyElementPropertiesEditionPart_TargetTableLabel;

	
	public static String ForeignKeyElementPropertiesEditionPart_PKColumnLabel;

	
	public static String ForeignKeyElementPropertiesEditionPart_CommentsLabel;

	
	public static String IndexElementPropertiesEditionPart_ColumnLabel;

	
	public static String IndexElementPropertiesEditionPart_AscLabel;

	
	public static String IndexElementPropertiesEditionPart_CommentsLabel;

	
	public static String ConstraintPropertiesEditionPart_NameLabel;

	
	public static String ConstraintPropertiesEditionPart_ExpressionLabel;

	
	public static String ConstraintPropertiesEditionPart_CommentsLabel;

	
	public static String SchemaPropertiesEditionPart_NameLabel;

	
	public static String SchemaPropertiesEditionPart_CommentsLabel;

	
	public static String SequencePropertiesEditionPart_NameLabel;

	
	public static String SequencePropertiesEditionPart_StartLabel;

	
	public static String SequencePropertiesEditionPart_IncrementLabel;

	
	public static String SequencePropertiesEditionPart_MinValueLabel;

	
	public static String SequencePropertiesEditionPart_MaxValueLabel;

	
	public static String SequencePropertiesEditionPart_CommentsLabel;

	
	public static String PropertiesEditionPart_DocumentationLabel;

	
	public static String PropertiesEditionPart_IntegerValueMessage;

	
	public static String PropertiesEditionPart_FloatValueMessage;

	
	public static String PropertiesEditionPart_ShortValueMessage;

	
	public static String PropertiesEditionPart_LongValueMessage;

	
	public static String PropertiesEditionPart_ByteValueMessage;

	
	public static String PropertiesEditionPart_BigIntegerValueMessage;

	
	public static String PropertiesEditionPart_BigDecimalValueMessage;

	
	public static String PropertiesEditionPart_DoubleValueMessage;

	
	public static String PropertiesEditionPart_PropertiesGroupLabel;

	
	public static String PropertiesEditionPart_RequiredFeatureMessage;

	
	public static String PropertiesEditionPart_AddTableViewerLabel;

	
	public static String PropertiesEditionPart_EditTableViewerLabel;

	
	public static String PropertiesEditionPart_RemoveTableViewerLabel;

	
	public static String PropertiesEditionPart_AddListViewerLabel;

	
	public static String PropertiesEditionPart_RemoveListViewerLabel;

	// Start of user code for additionnal NLS Constants

// End of user code

	static {
		// initialize resource bundle
		NLS.initializeMessages(BUNDLE_NAME, DatabaseMessages.class);
	}

	
	private DatabaseMessages() {
		//protect instanciation
	}
}
